                ...
                code.indented(() => {
                    ...
                    typeDefGen(code, funcDef, hasPositionalLiterals
                        ...
                        ? hasNamedLiterals
                            ...
                            : "NamedArgs"
                        : "{}"},`, (0, genutil_1.frag) `${returnType.staticType}, ${generateReturnCardinality(funcName, params, funcDef.return_typemod, hasNamedParams, anytypes, funcDef.preserves_optionality)}`);
                });
                code.writeln([(0, builders_1.t) `>;`]);
            }
        }
...
function generateReturnCardinality(name, params, returnTypemod, hasNamedParams, anytypes, preservesOptionality = false) {
    ...
    if (name === "std::if_else") {
        return (`$.cardutil.multiplyCardinalities<` +
    ...
    if (name === "std::assert_exists") {
        return (`$.cardutil.overrideLowerBound<` +
    ...
    const paramCardinalities = cardinalities.map(param => {
        if (param.typemod === "SetOfType") {
            if (preservesOptionality) {
                return (`$.cardutil.overrideUpperBound<` +
    ...
    const cardinality = paramCardinalities.length
        ? paramCardinalities.length > 1
            ? paramCardinalities
                .slice(1)
                .reduce((cards, card) => `$.cardutil.multiplyCardinalities<${cards}, ${card}>`, paramCardinalities[0])
